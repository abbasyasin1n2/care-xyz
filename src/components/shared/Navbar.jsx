...
export default function Navbar() {
  ...
  const currentNavigation = session ? authenticatedNavigation : navigation;
        ...
        <div className="flex justify-between h-16">
          <div className="flex items-center">
            <Link href="/" className="flex items-center">
              <span className="text-2xl font-bold text-blue-600">Care</span>
              <span className="text-2xl font-bold text-gray-800">.xyz</span>
            </Link>
          </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="relative h-10 w-10 rounded-full">
                    <Avatar>
                      <AvatarImage src={session.user?.image || session.user?.photoURL || ''} alt={session.user?.name} />
